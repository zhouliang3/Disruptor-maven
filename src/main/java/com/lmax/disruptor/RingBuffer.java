...
{
    private static final int BUFFER_PAD;//inote Buffer数组填充
    private static final long REF_ARRAY_BASE;//inote Buffer数组起始基址
    private static final int REF_ELEMENT_SHIFT;//inote 2^n=每个数组对象引用所占空间，这个n就是REF_ELEMENT_SHIFT
    private static final Unsafe UNSAFE = Util.getUnsafe();

    static
    {
        //inote Object数组引用长度，32位为4字节，64位为8字节
        ...
        Sequencer sequencer)
    {
        ...
        //inote 保证buffer大小不小于1
        if (bufferSize < 1)
        {
            throw new IllegalArgumentException("bufferSize must not be less than 1");
        }
        //inote 保证buffer大小为2的n次方
        ...
        /**
         * inote 结构：缓存行填充，避免频繁访问的任一entry与另一被修改的无关变量写入同一缓存行
         * --------------
         * *   数组头   * BASE
         * *   Padding  * 128字节
         * * reference1 * SCALE
...
public final class RingBuffer<E> extends RingBufferFields<E> implements Cursored, EventSequencer<E>, EventSink<E>
{
    ...
    /**
     * inote SequenceBarrier我们之后会详讲，这里我们可以理解为用来协调消费者消费的对象。例如消费者A依赖于消费者B，就是消费者A一定要后于消费者B消费，也就是A只能消费B消费过的，也就是A的sequence一定要小于B的。这个Sequence的协调，通过A和B设置在同一个SequenceBarrier上实现。同时，我们还要保证所有的消费者只能消费被Publish过的。
